...
// Hjelp: hent øvelsesnavn pr. økt
function ovelsesListeFor(okt){
  const arr = okt === 'Økt A' ? program.A : program.B;
  return arr.map(x => `${x.cat}: ${x.navn}`);
}
...
function renderLog(){
  ...
  arr.forEach((e,idx)=>{
    ...
    li.innerHTML = `
      <div><strong>${e.okt}</strong>${e.ovelse?` – ${e.ovelse}`:''} ${e.vekt?('· '+e.vekt+' kg'):''} ${e.tid?('· '+e.tid+' s hang'):''} ${e.reps?('· '+e.reps+' reps'):''} ${e.rir?('· RIR '+e.rir):''}</div>
...
// Øvelse-dropdown dynamikk
const feltOkt = document.getElementById('feltOkt');
const feltOvelse = document.getElementById('feltOvelse');
function repopulateOvelser(){
  const names = ovelsesListeFor(feltOkt.value);
  feltOvelse.innerHTML = '';
  names.forEach(n => {
    const opt = document.createElement('option');
    opt.textContent = n; opt.value = n; feltOvelse.append(opt);
  })
}
repopulateOvelser();
feltOkt.addEventListener('change', repopulateOvelser);
...
f.addEventListener('submit', (ev)=>{
  ...
  const entry = {
    ...
    ovelse: document.getElementById('feltOvelse').value,
...
document.getElementById('eksportCsv').addEventListener('click',()=>{
  const rows = [['dato','okt','ovelse','vekt','tid_s','reps','rir','notater']];
  for(const e of loadLog()){
    rows.push([e.dato, e.okt, e.ovelse||'', e.vekt??'', e.tid??'', e.reps??'', e.rir??'', (e.notater||'').replace(/\n/g,' ') ]);
  }
